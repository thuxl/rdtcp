...
public:
  void CwndChange1 (uint32_t oldCwnd, uint32_t newCwnd); //xl for debug.
  //for deadline and flow id to calculate missed rate and flow completion time, by xl
  uint32_t    m_deadline; //10^9 ns == 2^30 ns
  uint32_t	  m_flowid;
  
  //for protocols set mode
