...
//for ICTCP and RTCP
public:
  virtual void SentAck (SequenceNumber32 ack, uint32_t distance, bool fin_sign); //xl for receiver to estimate RTT
  virtual bool OnSeq (SequenceNumber32 Seq); //xl for receiver to estimate RTT
  virtual void  Measurement_r (Time t) = 0; //xl
  virtual Time RetransmitTimeout_r (uint32_t type) = 0; //xl type=0 is for rto_r, type=1 is for delacktimeout
  virtual void IncreaseMultiplier_r (); //xl
  virtual void ResetMultiplier_r (); //xl
  Time GetCurrentEstimate_r (void) const;

  uint16_t     m_multiplier_r;              // RTO Multiplier xl
  SequenceNumber32   m_last_ack;               // xl for receiver rtt
  RttHistory_t       m_history_r;       //xl  List of sent ack
  uint32_t           m_nSamples_r;               // Number of samples
  Time               m_currentEstimatedRtt_r;  //xl  Current estimate
...
private:
  ...
  //rtcp or ictcp, xl
public:
  void Measurement_r (Time m);
  Time RetransmitTimeout_r (uint32_t type); //xl
  
  Time         m_variance_r;   // Current variance
