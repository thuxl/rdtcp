...
Ipv4L3Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Ipv4L3Protocol")
	...
	//for offer load , by xl
	.AddAttribute ("Offer_Load", 
                   "for offered load ",
                   UintegerValue (64),
                   MakeUintegerAccessor (&Ipv4L3Protocol::m_offered_load),
                   MakeUintegerChecker<uint8_t> ())
  ;
  return tid;
}
                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  EcnTag ecntag;		//network/model/ecn-tag.h&cc by Balajee xl
  //packet->PeekPacketTag (ecntag);
  if (packet->RemovePacketTag(ecntag))
  {

    ipHeader.SetEcn(Ipv4Header::ECN_CE);
	NS_LOG_DEBUG ("find ecn, ecn removed from packet, set ecn = " << ipHeader.GetEcn());//xl
  } 
  else
  {
	//NS_LOG_DEBUG ("ecn not removed from packet!");//xl
  }
      ...
      if (outInterface->IsUp ())
        {
          NS_LOG_LOGIC ("Send to gateway " << route->GetGateway () 
				  ...
				  <<", outInterface->GetDevice ()->GetMtu ()="<<outInterface->GetDevice ()->GetMtu ()
				  );
...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  Ptr<Ipv4Interface> outInterface = GetInterface (interface);
  Time offerload_delay = Seconds ((double)80 * m_offered_load/100 * 
		  StaticCast<PointToPointNetDevice> (outInterface->GetDevice ())->GetDataRate ().CalculateTxTime (
			  outInterface->GetDevice ()->GetMtu () ) ); //by xl
  if (m_offered_load)
  NS_LOG_DEBUG (Simulator::Now ().GetSeconds () 
		  << " [node " << m_node->GetId () <<  ","<< this
		  << "], offer load=" << (uint16_t)m_offered_load 
		  << ", rate= "<<StaticCast<PointToPointNetDevice> (outInterface->GetDevice ())->GetDataRate ().GetBitRate()
		  << ", mtu= " <<outInterface->GetDevice ()->GetMtu ()
		  << ", offer load delay=" << offerload_delay 
		  << ", ipheader:"<< header << ", packet size="<< p->GetSize()
		  );
  Simulator::Schedule (offerload_delay ,
                       &Ipv4L3Protocol::SendRealOut , this,
					   rtentry, packet, ipHeader);
