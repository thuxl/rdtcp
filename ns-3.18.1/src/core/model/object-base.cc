      ...
      for (uint32_t i = 0; i < tid.GetAttributeN (); i++)
        {
          ...
          NS_LOG_DEBUG ("try to construct \""<< tid.GetName ()<<"::"<<
                        info.name <<"\",info.initialValue= "<<info.initialValue->SerializeToString(info.checker));
...
#ifdef HAVE_GETENV
              NS_LOG_DEBUG ("**********not found, search NS_ATTRIBUTE_DEFAULT"); 
          ...
          if (!found)
            {
              ...
              NS_LOG_DEBUG ("**********not found, set initial value."); 
                   ...
                   const AttributeValue &value)
{
  ...
  NS_LOG_DEBUG ("*-*-*-*-*-*-*-"<<this <<","<< accessor <<","<< checker<<"," <<&value<<","<<value.SerializeToString(checker)); 
