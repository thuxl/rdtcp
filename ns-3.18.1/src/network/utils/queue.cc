...
Queue::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::Queue")
	...
	//xl
    .AddTraceSource ("QueueBytes",
                     "The Bytes of Queue",
                     MakeTraceSourceAccessor (&Queue::m_nBytes))
    .AddTraceSource ("QueuePackets",
                     "The Packets of Queue",
...
{
  NS_LOG_FUNCTION (this);
  uint32_t tmp_m_nPackets = m_nPackets; 
  NS_LOG_LOGIC ("returns " << (tmp_m_nPackets == 0));
  return tmp_m_nPackets == 0;
}
