...
#include "ns3/nstime.h" //xl add for ictcp
//#include "socket.h" //xl add for rdtcp
...
*/
  ...
  //for ictcp
  uint64_t m_received_devicedataN;

  virtual void SetReceivedDataNum(uint64_t n)
  {
	  m_received_devicedataN = n;
  }

 virtual  uint64_t GetReceivedDataNum()
  {
	  return m_received_devicedataN ;
  }

 //for ictcp
  uint16_t		m_dev_socket_cout;  //xl for ictcp
  uint16_t		m_dev_closed_cout;  //xl
  Time		    m_dev_global_T;
  uint64_t		m_dev_received_coarsedataN [ MAX_FLOWS_N ];
  Time		    m_dev_RTT_connection [ MAX_FLOWS_N ]; 
  uint32_t		m_dev_ictcp_receiver_win [ MAX_FLOWS_N ];
  double		m_dev_BWA;  //Mbps
  uint8_t		m_dev_global_mode;  //1 for first slot, 2 for second slot.
  uint8_t		m_dev_global_not_start;  //xl
  uint8_t		m_dev_global_not_initial;  //xl
 
  
  // for rdtcp controller.
  struct rdtcp_info{
	  uint32_t   m_rec_get_flowsize;
	  double     m_rec_get_flowsize_Reciprocal;
	  uint32_t   m_rdtcp_max_win; //in packet unit
	  int16_t    m_rdtcp_delay_ack_rtts;
	  uint16_t   m_block;
	  Time		 m_connection_rtt;
	  uint8_t    m_exchange_id; //for two set of lowest weight wins. 0 is first set; 1 is second set; 2 is normal:no need to exchange,
	  uint32_t   m_deadline;
	  //uint32_t   m_rec_order;
  };
  //typedef std::map<uint32_t, uint32_t >::iterator ReceiverIterator;
  std::map<uint64_t, struct rdtcp_info > m_rec_infos;
  Time   								 m_global_rtt;
  uint16_t								 m_block_num;
  uint16_t								 m_global_block_id;
  uint8_t								 m_global_exchange_id;
  uint16_t		                         m_rdtcp_closed_count;  //xl
  uint16_t								 m_sendwin_in_rtt;

  //rdtcp controller list
  struct item_t {
	Ptr<Object>    m_socket_ptr;
	uint8_t        m_flags ;
	uint64_t       m_this_ptr ;
	uint16_t       m_insert_before_times ;
	uint32_t       m_rec_order;
  };

  std::list<struct item_t>               m_rdtcp_controller_list;
  int32_t								 m_flight_wins;
  
  //for all protocols
  uint32_t		  						 m_all_connections;
