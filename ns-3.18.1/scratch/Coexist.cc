...
uint32_t        longflowsize              = 10; //(MB). in 1000 nodes and 5 apps and 50kb short flows data center. due to 5x/(990*50+5x)=0.75, x=29700 kb
uint16_t			uniform_distr		  	  = 1;
uint16_t			num_of_flows_of_each_protocols		  	  = 1;
...
//  cmd.AddValue ("ThresholdFabric",        "Threshold of Fabric queue",                    ThresholdFabric);
  ...
  cmd.AddValue ("num_of_flows_of_each_protocols",  "num of flows to contend in bottleneck.",  num_of_flows_of_each_protocols);

  cmd.Parse (argc,argv);

  MachinesPerRack = num_of_flows_of_each_protocols * 2 + 1;
  NumberofRacks = 1;
...
//  PRINT_SIMPLE("ThresholdFabric                   : " << ThresholdFabric);
  ...
  PRINT_SIMPLE("longflowsize                      : " << longflowsize << "M");
  PRINT_SIMPLE("uniform_distr                     : " << uniform_distr);
  PRINT_SIMPLE("num_of_flows_of_each_protocols    : " << num_of_flows_of_each_protocols);
 ...
 // else delack_timeout            = rto_min;
  ...
  for (uint32_t i=0;i<NumberofApps;i++)
  {
    ExponentialVariable tmp (InterarrivalMultiplier * Start_Interval[i], 2 * InterarrivalMultiplier * Start_Interval[i]); //us
    StartDelay.push_back(tmp);
    ExponentialVariable tmp2 (1000, 2 * 1000); // start a long flow every 1 msec on avg
  ...
  //uint16_t sinknodes[NumberofApps];
  ...
  //uint32_t flowsize = 0; 
  //UniformVariable usize (2, 98); 
  //UniformVariable usize (20, 60); 
  //Ptr<ParetoRandomVariable> psize = CreateObject<ParetoRandomVariable> ();
  //psize->SetAttribute ("Mean", DoubleValue (50));
  //psize->SetAttribute ("Bound", DoubleValue (100));
  ...
  //{
   // NodeIdx = App2Node[i];
    ...
    //{
      ...
      PRINT_SIMPLE("============================================================");
      PRINT_SIMPLE("============================================================");
      PRINT_SIMPLE("[0] other protocol Sink: " << SinkNodeIdx );
      PacketSinkHelper sink ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), OTHER_FLOW_PORT));
      ...
      ApplicationContainer sinkApps1 = sink.Install (nEnd[SinkNodeIdx ]);
      sinkApps1.Start  (Seconds (0.0));


      PRINT_SIMPLE("============================================================");
      PRINT_SIMPLE("============================================================");
      PRINT_SIMPLE("[0] TCP Sink: " << SinkNodeIdx );
      PacketSinkHelper long_flow_sink ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), LONG_FLOW_PORT));
      long_flow_sink.SetAttribute ("DCTCP_MODE", UintegerValue (0)); // by xl
      long_flow_sink.SetAttribute ("RTCP_MODE",  UintegerValue (0)); // by xl
      long_flow_sink.SetAttribute ("ICTCP_MODE", UintegerValue (0)); // by xl
      long_flow_sink.SetAttribute ("RDTCP_MODE", UintegerValue (0)); // by xl
      long_flow_sink.SetAttribute ("RDTCP_MAXQUEUE_SIZE", UintegerValue (0)); // by xl
      long_flow_sink.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (0)); // by xl

      ApplicationContainer sinkApps2 = long_flow_sink.Install (nEnd[SinkNodeIdx ]);
      sinkApps2.Start  (Seconds (0.0));
     
	  // remeber this node idx as sink (target for other app nodes)
      //SinkNodeIdx = NodeIdx;
      //CountSinkIdx = Count;
	  //sinknodes[Count] = NodeIdx;
      ...
      //int j = 0;
      //while (delay_prev < NanoSeconds  (Stop))
      //{
        // create a new start delay for each app
        //delay.push_back(delay_prev + MicroSeconds (StartDelay[Count].GetValue()));
        //delay.push_back(delay_prev + MilliSeconds (10)); //for incast, the first app is at 10ms, not 0ms.
        //delay_prev = delay[j];
        //j++;
      //}
      //long_flow_delay.clear();
      //long_flow_delay_prev = MilliSeconds(0.0);
      //j = 0;
      //long_flow_delay.push_back(long_flow_delay_prev);   //add by xl for longflow
      //while (long_flow_delay_prev < NanoSeconds (Stop))
      //{
        //long_flow_delay.push_back(long_flow_delay_prev + MicroSeconds (LongFlowStartDelay[Count].GetValue()));
    ...
    //{
      ...
      //if (first_leaf) {
        //first_leaf = false;
		NodeIdx = 1;
        for (uint32_t j = 0; j < num_of_flows_of_each_protocols; j++) {
          PRINT_SIMPLE("[" << NodeIdx << "] TCP Flow Source Node "<< NodeIdx <<": ---> Sink Node " << SinkNodeIdx
				  <<", ip: "<< nEnd[SinkNodeIdx]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()
				  );
          ...
          source.SetAttribute ("MaxBytes", UintegerValue (longflowsize * 1024 * 1024)); 
          source.SetAttribute ("SendSize", UintegerValue (longflowsize * 1024 * 1024)); // cannot call send() for size larger than TCP buffer = 128 MB
      ...
      source.SetAttribute ("DCTCP_MODE", UintegerValue (0)); // by xl
      source.SetAttribute ("RTCP_MODE",  UintegerValue (0)); // by xl
      source.SetAttribute ("ICTCP_MODE", UintegerValue (0)); // by xl
      source.SetAttribute ("RDTCP_MODE", UintegerValue (0)); // by xl
      source.SetAttribute ("RDTCP_MAXQUEUE_SIZE", UintegerValue (0)); // by xl
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (0)); // by xl

          ApplicationContainer sourceApps = source.Install (nEnd[NodeIdx]);
          PRINT_SIMPLE("Start longflow at " << NodeIdx << " at time = " << Seconds(0.0) <<" flowid= "<<num_flows
				  << " size: "<<longflowsize*1024*1024 <<" B");
        ...
        for (uint32_t j=0; j<num_of_flows_of_each_protocols; j++)
        {
          PRINT_SIMPLE("[" << NodeIdx  << "] other flow Source Node " << NodeIdx << ": ---> Sink Node " << SinkNodeIdx 
				  <<", ip: "<< nEnd[SinkNodeIdx]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()
				  );
          BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress(nEnd[SinkNodeIdx]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), OTHER_FLOW_PORT));
          source.SetAttribute ("MaxBytes", UintegerValue (longflowsize * 1024 * 1024));
          source.SetAttribute ("SendSize", UintegerValue (longflowsize * 1024 * 1024));
      ...
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (RDTCP_CONTROLLER_Mode)); // by xl
		  ...
		  //delaytime += 5;
          PRINT_SIMPLE("Start app at " << NodeIdx << " at time = " << Seconds(0.0) <<" flowid= "<<num_flows
				  << " size: "<<longflowsize * 1024 * 1024 <<" B");
          sourceApps.Start  (Seconds(0.0) );
  ...
  //Ptr<FlowMonitor> monitor = flowmonhelp.Install(nEnd[1]);
    ...
    Print_st = 1;
    if (Print_st)
	//for (uint8_t k=0; k<NumberofApps; k++) 
	{
  Ptr<Queue> tor_to_sink_device_queue = nToR[0]->GetDevice( 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
  //Ptr<Queue> fabric_to_sinkToR_device_queue = nFabric->GetDevice(ToRi_under_Fabric + 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
  ...
  //fabric_to_sinkToR_device_queue ->TraceConnectWithoutContext ("QueueBytes", MakeCallback (QueueBytes )); //xl
  //fabric_to_sinkToR_device_queue ->TraceConnectWithoutContext ("QueuePackets", MakeCallback (QueuePackets )); //xl
  //fabric_to_sinkToR_device_queue ->TraceConnectWithoutContext ("Drop", MakeCallback (DropTrace )); //xl
  //fabric_to_sinkToR_device_queue ->TraceConnectWithoutContext ("Enqueue", MakeCallback (EnQTrace )); //xl
  //fabric_to_sinkToR_device_queue ->TraceConnectWithoutContext ("Dequeue", MakeCallback (DeQTrace )); //xl
	}
  ...
  //monitor->SerializeToXmlFile("lab-1.flowmon", true, true); by xl on Tue May 28 17:02:02 CST 2013
    ...
    if (Print_st)
    {
	   ...
	   //for incast 
	   for (uint8_t k=0; k<NumberofRacks; k++) {
        Ptr<Queue> tor_to_sink_device_queue = 
			nToR[k]->GetDevice( 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
		cout <<"sink [ "<< SinkNodeIdx <<" ], sink ip: "<< nEnd[SinkNodeIdx ]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal() 
			<< ", tor ip:" <<nToR[SinkNodeIdx ]->GetObject<Ipv4>()->GetAddress( 1 ,0).GetLocal() 
			<<endl;
