...
uint32_t        flowsize                  = 32; //in kb unit
...
//  cmd.AddValue ("ThresholdFabric",        "Threshold of Fabric queue",                    ThresholdFabric);
  ...
  cmd.AddValue ("flowsize",               "flowsize to send (KB)",                             flowsize );
...
//  PRINT_SIMPLE("ThresholdFabric                   : " << ThresholdFabric);
  ...
  PRINT_SIMPLE("flowsize                          : " << flowsize);
    ...
    else
    {
      ...
      if (first_leaf) {
        ...
        for (uint32_t j = 0; j < long_flow_delay.size(); j++) {
          PRINT_SIMPLE("[" << i << "] Long Flow Source Node " << NodeIdx << ": ---> Sink Node " << SinkNodeIdx << ", size: 4024 * 1024B");
          ...
          source.SetAttribute ("MaxBytes", UintegerValue (8 * 1024 * 1024)); // send 1 MB
          source.SetAttribute ("SendSize", UintegerValue (8 * 1024 * 1024)); // cannot call send() for size larger than TCP buffer = 128 MB
      ...
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (RDTCP_CONTROLLER_Mode)); // by xl
          ...
          PRINT_SIMPLE("Start longflow at " << NodeIdx << " (Iteration = " << j << "), at time = " << long_flow_delay[j]);
        ...
        {
          PRINT_SIMPLE("[" << i << "] Source Node " << NodeIdx << ": ---> Sink Node " << SinkNodeIdx << ", size: "<<flowsize *1024 <<" B");
      ...
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (RDTCP_CONTROLLER_Mode)); // by xl
		  ...
		  //delaytime += 5;
          PRINT_SIMPLE("Start app at " << NodeIdx << " (Iteration = " << j << "), at time = " << delay[j]);
