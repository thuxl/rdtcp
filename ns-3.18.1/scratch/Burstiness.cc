...
uint32_t        longflowsize              = 30; //(MB). in 1000 nodes and 5 apps and 50kb short flows data center. due to 5x/(990*50+5x)=0.75, x=29700 kb
...
uint32_t MaxBytesPerApp[] = {50 * 1024,   32 * 1024,   32 * 1024,   32 * 1024,   32 * 1024};
...
//  PRINT_SIMPLE("ThresholdFabric                   : " << ThresholdFabric);
 ...
 // else delack_timeout            = rto_min;
    ...
    else
    {
      ...
      if (first_leaf) {
        ...
        for (uint32_t j = 0; j < long_flow_delay.size(); j++) {
          PRINT_SIMPLE("[" << i << "] Long Flow Source Node " << NodeIdx << ": ---> Sink Node " << SinkNodeIdx
      ...
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (RDTCP_CONTROLLER_Mode)); // by xl
          ...
          PRINT_SIMPLE("Start longflow at " << NodeIdx << " (Iteration = " << j << ") at time = " << long_flow_delay[j].GetNanoSeconds()<<" flowid= "<<num_flows
        ...
        {
            //if (uniform_distr == 1 )
			 // flowsize = usize.GetInteger (2, 98);
			//else
			 // flowsize = psize->GetInteger ();
          PRINT_SIMPLE("[" << i << "] Source Node " << NodeIdx << ": ---> Sink Node " << SinkNodeIdx 
				  <<", ip: "<< nEnd[SinkNodeIdx]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()
				  );
      ...
      source.SetAttribute ("RDTCP_CONTROLLER_MODE", UintegerValue (RDTCP_CONTROLLER_Mode)); // by xl
		  ...
		  //delaytime += 5;
          PRINT_SIMPLE("Start app at " << NodeIdx << " (Iteration = " << j << ") at time = " << delay[j].GetNanoSeconds()<<" flowid= "<<num_flows
				  << " size: "<< MaxBytesPerApp[CountSinkIdx] <<" B");
  ...
  //Ptr<FlowMonitor> monitor = flowmonhelp.Install(nEnd[1]);
	...
	{
		//uint8_t k=2;
		uint8_t ToRi = sinknodes[k] / MachinesPerRack ;
		uint8_t Endi_under_ToR = sinknodes[k] % MachinesPerRack ;
		uint8_t ToRi_under_Fabric = ToRi ;
  Ptr<Queue> tor_to_sink_device_queue = nToR[ToRi]->GetDevice(Endi_under_ToR + 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
  ...
  //monitor->SerializeToXmlFile("lab-1.flowmon", true, true); by xl on Tue May 28 17:02:02 CST 2013
    ...
    if (Print_st)
    {
	   ...
	   //for incast 
	   for (uint8_t k=0; k<NumberofApps; k++) {
		uint16_t ToRi = sinknodes[k] / MachinesPerRack ;
		uint16_t Endi_under_ToR = sinknodes[k] % MachinesPerRack ;
		uint16_t ToRi_under_Fabric = ToRi ;
        Ptr<Queue> tor_to_sink_device_queue = 
			nToR[ToRi]->GetDevice(Endi_under_ToR + 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
        Ptr<Queue> fabric_to_sinkToR_device_queue = 
	        nFabric->GetDevice(ToRi_under_Fabric + 1 )->GetObject <PointToPointNetDevice> ()->GetQueue(); 
		cout <<"sink [ "<< (uint16_t)sinknodes[k]<<" ], sink ip: "<< nEnd[sinknodes[k]]->GetObject<Ipv4>()->GetAddress(1,0).GetLocal() 
			<< ", ToRi or ToRi_under_Fabric ="<<ToRi 
			<< ", tor ip:" <<nToR[ToRi]->GetObject<Ipv4>()->GetAddress(Endi_under_ToR + 1 ,0).GetLocal() 
			<< ", Endi_under_ToR ="<< Endi_under_ToR <<endl;
		  ...
		  cout << "\tFabric " << StaticCast<DropTailQueue> (fabric_to_sinkToR_device_queue)->Get_Droptimes ()  
			  << " drops due queue full at bottleneck" << endl;
